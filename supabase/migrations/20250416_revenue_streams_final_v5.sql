...
select 
    (select id from public.revenue_streams where name = 'Default' limit 1),
    id,
    'owner',
    true
from auth.users;
...
begin
    ...
    select id into default_stream_id from public.revenue_streams where name = 'Default' limit 1;
    ...
    if exists (select 1 from pg_tables where schemaname = 'public' and tablename = 'customers') then
        ...
        create policy "customers_insert" on public.customers for insert with check (user_is_stream_member(stream_id));
        create policy "customers_update" on public.customers for update using (user_is_stream_member(stream_id));
        create policy "customers_delete" on public.customers for delete using (user_is_stream_member(stream_id));
    end if;
    ...
    if exists (select 1 from pg_tables where schemaname = 'public' and tablename = 'tasks') then
        ...
        create policy "tasks_insert" on public.tasks for insert with check (user_is_stream_member(stream_id));
        create policy "tasks_update" on public.tasks for update using (user_is_stream_member(stream_id));
        create policy "tasks_delete" on public.tasks for delete using (user_is_stream_member(stream_id));
    end if;
    ...
    if exists (select 1 from pg_tables where schemaname = 'public' and tablename = 'deals') then
        ...
        create policy "deals_insert" on public.deals for insert with check (user_is_stream_member(stream_id));
        create policy "deals_update" on public.deals for update using (user_is_stream_member(stream_id));
        create policy "deals_delete" on public.deals for delete using (user_is_stream_member(stream_id));
    end if;
    ...
    if exists (select 1 from pg_tables where schemaname = 'public' and tablename = 'prospects') then
        ...
        create policy "prospects_insert" on public.prospects for insert with check (user_is_stream_member(stream_id));
        create policy "prospects_update" on public.prospects for update using (user_is_stream_member(stream_id));
        create policy "prospects_delete" on public.prospects for delete using (user_is_stream_member(stream_id));
    end if;
    ...
    if exists (select 1 from pg_tables where schemaname = 'public' and tablename = 'boards') then
        ...
        create policy "boards_insert" on public.boards for insert with check (user_is_stream_member(stream_id));
        create policy "boards_update" on public.boards for update using (user_is_stream_member(stream_id));
        create policy "boards_delete" on public.boards for delete using (user_is_stream_member(stream_id));
    end if;
    ...
    if exists (select 1 from pg_tables where schemaname = 'public' and tablename = 'board_columns') then
        ...
        create policy "board_columns_insert" on public.board_columns for insert with check (user_is_stream_member(stream_id));
        create policy "board_columns_update" on public.board_columns for update using (user_is_stream_member(stream_id));
        create policy "board_columns_delete" on public.board_columns for delete using (user_is_stream_member(stream_id));
    end if;
    ...
    if exists (select 1 from pg_tables where schemaname = 'public' and tablename = 'board_entries') then
        ...
        create policy "board_entries_insert" on public.board_entries for insert with check (user_is_stream_member(stream_id));
        create policy "board_entries_update" on public.board_entries for update using (user_is_stream_member(stream_id));
        create policy "board_entries_delete" on public.board_entries for delete using (user_is_stream_member(stream_id));
    end if;
end $$;
