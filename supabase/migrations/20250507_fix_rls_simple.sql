...
CREATE POLICY "Enable insert for stream members" ON people
    FOR INSERT
    WITH CHECK (
        EXISTS (
            SELECT 1 
            FROM revenue_stream_members rsm
            WHERE rsm.stream_id = NEW.stream_id
            AND rsm.user_id = auth.uid()
        )
    );

-- Create update policy
CREATE POLICY "Enable update for stream members" ON people
    FOR UPDATE
    USING (
        EXISTS (
            SELECT 1 
...
CREATE POLICY "Enable delete for stream members" ON people
    FOR DELETE
    USING (
        EXISTS (
            SELECT 1 
